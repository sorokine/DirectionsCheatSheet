...
\begin{tikzpicture}
  ...
  \draw (w) node[above left,  align=right]  {west\\left\\prev\\back};
  ...
  \foreach \tile in {0,1,...,15} {
    \pgfmathparse{13.5 - 3 * mod(\tile,4)} \let\trow\pgfmathresult
    \pgfmathparse{4.5 + 3 * int(\tile/4)} \let\tcol\pgfmathresult
