...
\begin{tikzpicture}
  ...
  \draw[->, very thick] (center) -- (N) node[above] {{\Huge\fontencoding{T1}\fontfamily{pzc}\selectfont{}N}$_0$};
  \draw[->, very thick] (ne) -- (NE) node[above] {{\Huge\fontencoding{T1}\fontfamily{pzc}\selectfont{}NE}$_1$};
  \draw[->, very thick] (center) -- (E) node[above right] {{\Huge\fontencoding{T1}\fontfamily{pzc}\selectfont{}E}$_2$};
  ...
  \draw (n) node[above right, align=left]  {north\\above\\top\\up};
  \draw (n) node[above left,  align=right] {север\\над\\верх};
  \draw (e) node[above right, align=left]  {east\\right\\next\\ahead};
  \draw (e) node[below right, align=right] {восток\\справа\\вперед};
  \draw (s) node[below right, align=left]  {south\\below\\bottom\\down};
  \draw (s) node[below left,  align=right] {юг\\под\\низ};
  \draw (w) node[above left,  align=left]  {west\\left\\prev\\back};
  \draw (w) node[below left, align=right]  {запад\\слева\\назад};
  ...
  \foreach \col[count=\coli from 0] in {3,3.5,4,4.5,5,5.5}
    \draw (\col,15) node [below right] {\coli};
  \foreach \row[count=\rowi from 1] in {14.5,14,13.5,13,12.5}
    \draw (3,\row) node [below right] {\rowi};
